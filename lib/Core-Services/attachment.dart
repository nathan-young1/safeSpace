...
class _AttachmentState extends State<Attachment> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: StreamBuilder(
        stream: FirestoreFileStorage.streamAttachmentList(collection: widget.collection,dbName: widget.dbName),
        builder: (context,fileAttachments) => Scaffold(
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              child: IconButton(
                ...
                collection: widget.collection,checkIfExist: fileAttachments.data,commandFrom: UploadFileToFirestore.fromAttachment)
        ...
        body: (fileAttachments.data != null)
            ? (fileAttachments.data.length != 0)?ListView.builder(
                itemCount: fileAttachments.data.length,
                itemBuilder: (context, index) {
                  return Padding(
                    padding: EdgeInsets.fromLTRB(10.w,(context.isMobileTypeTablet)?15.h:5.h,6.w,6),
                    child: GestureDetector(
                      onTap: () async {
                          await openfile(widget.collection, widget.docName,fileAttachments.data[index]);
                        },
                        child: Row(
                         mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                          children: [
                            fileExists(collection: widget.collection,documentName: widget.docName,fileName: fileAttachments.data[index])
                            ? Icon(MdiIcons.fileCheck, size: 30.r, color: mainColor)
                            : Icon(MdiIcons.fileDownload, size: 31.r, color: mainColor),
                            SizedBox(width: 7.w),
                          Expanded(child: Text(fileAttachments.data[index],style: TextStyle(fontSize: RFontSize.normal))),
                          ...
                          onSelected: (Options result) async {
                            switch (result) {
                              ...
                              if(await showConfirmFileDeleteDialog(context,DeleteOption.file)){
                               await deleteFromFirestore(widget.collection,widget.dbName,fileAttachments.data[index]);
                               }
                                break;
                            }
                          },
          ...
          bottomSheet: (fileAttachments.data != null && fileAttachments.data.length != 0)
            ? GestureDetector(
                onTap: () async {
                  await getFiles(collection: widget.collection,documentName: widget.docName,fileDir: fileAttachments.data);
                  setState(() {});
                },
                child: Container(
                  height: 50.h,
